...
func (c Charts) Hit(userid, date string, url string, weigth float64) {
	t := c.parseDate(date)
	c.store.update(c.dayKey(userid, t), url, weigth)
	c.store.update(c.monthKey(userid, t), url, weigth)
	c.store.update(c.globalKey(userid, t), url, weigth)
}
